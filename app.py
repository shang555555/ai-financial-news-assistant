...
def main() -> None:
    ...
    if not settings.news_api_key:
        st.error("未检测到 `NEWS_API_KEY`，请先在 `.env` 中配置。")
        st.stop()

    if not settings.deepseek_api_key and not settings.openai_api_key:
        st.error("未检测到 `DEEPSEEK_API_KEY` 或 `OPENAI_API_KEY`，请先在 `.env` 中配置。")
        st.stop()
    ...
    analysis_service = NewsAnalysisService(settings)

    with st.spinner("正在获取新闻并进行分析，请稍候..."):
        run_started_at = time.perf_counter()
        try:
            articles = news_service.fetch_news(stock_symbol=stock_symbol, limit=max_articles)
